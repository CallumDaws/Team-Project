...
public class TreasureHuntAPI {
	...
	public String createTeam(String playerName, String teamName) throws MalformedURLException, IOException {
		...
		in.close();
		conn.disconnect();
		String token = assignTeamLeader(playerName, result);
		// Assign team leader
		if (!token.equals("error")) {
			System.out.println("Token from server: " + token);
			setToken(token); // Store authentication token
			setPlayerName(playerName); // Store player name
			return result;
		} else {
			return "error";
		}
