	...
	public void updateScores() {
		...
		try {
			...
			for (String key : scores.keySet()) {
				...
				Team tempTeam = new Team(teamDetails.get("teamid").toString(), teamDetails.get("colour").toString(), teamDetails.get("teamleader").toString(), teamDetails.get("score").getAsInt());
				 teamArray.add(tempTeam);
			}
			Collections.sort(teamArray, Team.compare());
			System.out.println(teamArray.get(0).getScore());
