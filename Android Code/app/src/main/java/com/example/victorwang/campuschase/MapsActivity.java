...
public class MapsActivity extends FragmentActivity implements OnMapReadyCallback {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        updateGPSTimer = new Timer();
        updateGPSTimer.scheduleAtFixedRate(new TimerTask() {
            @Override
            public void run() {
                updateLoop();
            }
        ...
        updateScoreTimer.scheduleAtFixedRate(new TimerTask() {
            @Override
            public void run() {
    ...
    private void updatePlayerLocations() {
        ...
        if (gps.canGetLocation()) {
            ...
            //playerLocation = new LatLng(gps.getLatitude(), gps.getLongitude());
             runOnUiThread(new Runnable() {
               @Override
             public void run() {
            ...
            if (!markers.containsKey(api.getPlayerName())) {
                ...
                markerpos.position(gps.getLocation());
                markerpos.title(api.getPlayerName());
                playerMarker = mMap.addMarker(markerpos);
                mMap.moveCamera(CameraUpdateFactory.newLatLng(gps.getLocation()));
                markers.put(api.getPlayerName(), playerMarker);
            } else {
                oldPlayerMarker = markers.get(api.getPlayerName());
                System.out.println("location update:" + gps.getLocation().toString()+ "\nPlayerMarker: " + gps.getLocation().toString());
                playerMarker.remove();
                markerpos.position(gps.getLocation());
                markerpos.title(api.getPlayerName());
                playerMarker = mMap.addMarker(markerpos);
                markers.remove(oldPlayerMarker);
                markers.put(api.getPlayerName(),playerMarker);
            ...
            if (teamDetails == null) {
                ...
                for (int i = 0; i < players.size(); i++) {
                    tempPlayer = players.get(i).getAsJsonObject();
                    if (!tempPlayer.get("playername").getAsString().equals(api.getPlayerName())) {
                        ...
                        if (tempPlayer.get("location") != null) {
                            ...
                            tempLocation = new LatLng(lat, lng);
                            runOnUiThread(new Runnable() {
                                @Override
                                public void run() {
                            if (!markers.containsKey(tempPlayer.get("playername").getAsString())) {

                                        tempMarker = mMap.addMarker(new MarkerOptions().position(tempLocation).title(tempPlayer.get("playername").getAsString()));
                                        markers.put(tempPlayer.get("playername").getAsString(), tempMarker);


                            } else {
                                tempMarker = markers.get(tempPlayer.get("playername").getAsString());
                                oldTempMarker = tempMarker;
                                tempMarker.remove();
                                markerpos.position(tempLocation);
                                markerpos.title(tempPlayer.get("playername").getAsString());
                                tempMarker = mMap.addMarker(markerpos);
                                markers.remove(oldTempMarker);
                                markers.put(tempPlayer.get("playername").getAsString(), tempMarker);
                            }
                                }
                            });
